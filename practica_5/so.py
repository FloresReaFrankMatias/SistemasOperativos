...
class MemoryManager:
    ...
    def alocFrames(self,number):
        ...
        for i in range(number):
            frame = self._framesLibres.pop(0)
            frames.append(frame)
        return frames

    def freeFrames(self,lista): ##este for para que no se guarden como dictValues
        for i in lista: 
...
class Loader:
    ...
    def load(self, program):
        ...
        pageTable = dict()
        for pag, frame in enumerate(framesParaUsar):
            pageTable[pag] = frame
...
class Dispatcher():
    ...
    def load(self, pcb):
        ...
        for page, frame in pcb.pageTable.items():
